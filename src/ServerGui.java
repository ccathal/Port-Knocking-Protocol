...
public class ServerGui extends JFrame {
	...
	public ServerGui() {
		...
		frame.setPreferredSize(new Dimension(525, 410));
		...
		knock.setBorder(BorderFactory.createEmptyBorder(20, 10, 20, 10));
		knockLabel = new JLabel("Choose Knock Sequence Length");
		knockLabel.setFont(new Font("Arial", Font.PLAIN, 30));
		...
		for (int i = 3; i < 7; i++) {
			...
			button.setPreferredSize(new Dimension(120, 130));
			...
			button.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					// FRAME 2 :- containing input fields for server port, ipaddress and knock
					// sequence
					...
					portLabel.setFont(new Font("Arial", Font.PLAIN, 38));
					portBox = new JTextField();
					portBox.setFont(new Font("Arial", Font.PLAIN, 40));
					port.add(portLabel);
					port.add(portBox);
					port.setBorder(BorderFactory.createEmptyBorder(20, 10, 10, 10));

					// JPanel constructed for port knocks
					listing = new JPanel();
					listingLabel = new JLabel("Port Knocks");
					listingLabel.setFont(new Font("Arial", Font.PLAIN, 38));
					...
					// loop to create port knock input fields corresponding to the number on the
					// button previously clicked
						...
						public void actionPerformed(ActionEvent arg0) {
							try {
								...
								if (!isInteger(endPort, 10)) {
									accepted = false;
								} else if (!(Integer.parseInt(endPort) >= 0) || !(Integer.parseInt(endPort) < 65536))
									accepted = false;
								...
								if (startButton.getText() == "Start Server" && accepted) {
									...
									int input = JOptionPane.showConfirmDialog(null,
											"Connections will be lost and iptables will remain appended!",
