...
public class MyServer extends Thread {
	...
	public void runTCPDUmp() throws IOException, InterruptedException {
		...
		try {
			...
			while ((line = inputStreamReader.readLine()) != null) {
				...
				// boolean which will be implemented if IllegalArgumentException is thrown by failing to create a client or decryption error
				...
				try {	        		
	        		...
	        		logger.warning("Failure to create client identity (parsing client IP and/or port): IP - " + aks.getAddress() + ": Port - " + aks.getPort());
	        		parsingCheck = false;
	        	}
        		...
        		if(hashKnock.containsKey(aks)) {
        			...
        			if(!hashKnock.get(aks).isEmpty() && (arrivalTime/1000 - hashKnock.get(aks).get(hashKnock.get(aks).size() -1).getArrivalTime()/1000 > 20.0)) {
        		...
        		if(this.keyManagment.containsKey(InetAddress.getByName(srcIP))) {
	        		...
	        		if(verify(text, sign, this.keyManagment.get(InetAddress.getByName(srcIP)))) {
		        		try {
							...
							byte[] bytePlainText = aesCipher.doFinal(Base64.getDecoder().decode(packetData[0].toString()));
							...
							if(parsingCheck && arrivalTime/1000 - Long.parseLong(values[0])/1000 > 20.0) {
								logger.severe("Delivery of packet took too long. Assuming packet modification: IP - " + aks.getAddress() + ": Port - " + aks.getPort());
								hashKnock.get(aks).clear();
								parsingCheck = false;
							}
										
		        		} catch (IllegalArgumentException | BadPaddingException | NullPointerException | ArrayIndexOutOfBoundsException ex) {
		        			logger.warning("Failure decrypting client packet: IP - " + aks.getAddress() + " : Port - " + aks.getPort());
		        			parsingCheck = false;
		        		} 
	        		} else {
	        			logger.warning("Failure to authenticate client: IP - " + aks.getAddress() + " : Port - " + aks.getPort());
	        			parsingCheck = false;
	        		}
        		} else {
        			logger.warning("Server does not contain a key for client: IP - " + aks.getAddress() + " : Port - " + aks.getPort());
        			parsingCheck = false;
        		}
	        		
        		// check if no IllegalArgumentException errors have occurred or future packets sent
        		if(parsingCheck) {
	...
	// method verifies the clients signature of their private key on the encrypted data
	public boolean verify(String text, String signature, String pubKey) throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {	
