...
public class MyServer extends Thread {
		            ...
		            if (arr.size() < 5) {
		            	...
		            	if(arr.size() == 4) {
		            		...
		            		if (knockingSequence.equals(confirmKnockingSequence)) {
		            			...
		            			makeConnection(aks.getAddress(), aks.getPort(), connectionKnocks);
		            		} else {
	...
	private void makeConnection(InetAddress srcAddress, int srcPort, ArrayList<Integer> connectKnocks) throws InvalidKeyException, IllegalBlockSizeException, BadPaddingException, NoSuchAlgorithmException, NoSuchPaddingException {
		
		Process connUDP, connTCP, connICMP;
		Process dropUDP, dropTCP, dropICMP;
		String address = srcAddress.getHostAddress().toString();
		
		for(int port : connectKnocks) {
	    	...
	    	try {
				...
				logger.info("Server switched connection port - " + connectPort); 
